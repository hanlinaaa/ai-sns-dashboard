...
const initialBrandSettings: BrandSettings = {
  brandName: "サクサクSNS君",
  brandBio: "AIを活用した次世代のSNSコンテンツ生成ツールを提供するテクノロジー企業です。",
  targetAudience: "SNSマーケティング担当者、中小企業の広報担当、個人事業主",
  defaultTones: {
    x: "friendly",
    instagram: "promo",
    line: "business",
  },
  customInstructions: "・絵文字は適度に使用する\n・専門用語はできるだけ避け、わかりやすい言葉を使う",
  firstPerson: "弊社",
  secondPerson: "皆様",
  ngWords: ["他社ツール", "絶対", "100%", "完全"],
  mustHaveWords: ["サクサク", "時短"],
  allowHashtags: true,
  maxHashtags: {
    x: 3,
    instagram: 10,
  ...
  contactInfo: "support@example.com (平日10:00-18:00)",
  disclaimer: "※本キャンペーンは予告なく終了する場合がございます。",
}

const brandStorageKey = "sns-dashboard-brand-settings"

export default function BrandSettingsPage() {
  const [settings, setSettings] = useState<BrandSettings>(initialBrandSettings)
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        setSettings(JSON.parse(raw) as BrandSettings)
      }
    } catch {
      toast.error("保存済みブランド設定の読み込みに失敗しました")
    }
  }, [])
  ...
  const handleSave = async () => {
    ...
    window.localStorage.setItem(brandStorageKey, JSON.stringify(settings))
    setIsSaving(false)
    toast.success("ブランド設定を保存しました", {
      description: "次回のコンテンツ生成から適用されます。"
    })
  }
  ...
  const handleToneChange = (platform: "x" | "instagram" | "line", tone: any) => {
    setSettings(prev => ({
      ...prev,
  ...
  const handleMaxHashtagsChange = (platform: "x" | "instagram" | "line", count: number) => {
    setSettings(prev => ({
      ...prev,
        ...
        <div className="flex flex-col justify-between gap-4 rounded-lg border bg-card/95 p-4 shadow-sm sm:flex-row sm:items-center">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="truncate text-xl font-semibold text-foreground sm:text-2xl">ブランド設定</h1>
              <p className="mt-1 text-sm text-muted-foreground">AIが生成するコンテンツのトーンやルールを定義します</p>
            </div>
          </div>
          <Button onClick={handleSave} disabled={isSaving} className="h-10 w-full sm:w-auto">
            {isSaving ? (
            ...
            <Card className="rounded-lg border-border/70 shadow-sm">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">この情報は、AIがブランドの価値観に沿った文章を生成するのに役立ちます。</p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">コンテンツ生成画面でのデフォルト値として使用されます。</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="tone" className="space-y-6">
            <Card className="rounded-lg border-border/70 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>各プラットフォームのデフォルトの語り口や、独自のルールを設定します。</CardDescription>
              </CardHeader>
              <CardContent className="space-y-8">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2 rounded-lg border bg-muted/20 p-4">
                      ...
                      <Select value={settings.defaultTones.x} onValueChange={(v) => handleToneChange("x", v)}>
                    ...
                    <div className="space-y-2 rounded-lg border bg-muted/20 p-4">
                      ...
                      <Select value={settings.defaultTones.instagram} onValueChange={(v) => handleToneChange("instagram", v)}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="business">ビジネス敬語</SelectItem>
                          <SelectItem value="friendly">親しみやすい</SelectItem>
                    ...
                    <div className="space-y-2 rounded-lg border bg-muted/20 p-4">
                      <Label>LINE</Label>
                      <Select value={settings.defaultTones.line} onValueChange={(v) => handleToneChange("line", v)}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="business">ビジネス敬語</SelectItem>
                          <SelectItem value="friendly">親しみやすい</SelectItem>
                ...
                <div className="space-y-3">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">AIへの追加の指示です。生成されるすべての文章に影響します。</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="safety" className="space-y-6">
            <Card className="rounded-lg border-border/70 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>ブランドセーフティを守るための制限やフォーマットルールを設定します。</CardDescription>
              </CardHeader>
              <CardContent className="space-y-8">
                  ...
                  <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {settings.ngWords.map((word) => (
                        <Badge key={word} variant="destructive" className="flex items-center gap-1 pr-1">
                          {word}
                          <button onClick={() => handleRemoveWord("ngWords", word)} className="hover:bg-destructive-foreground/20 rounded-full p-0.5">
                            <X className="w-3 h-3" />
                          </button>
                        </Badge>
                      ))}
                    ...
                    />
                    <p className="text-xs text-muted-foreground">競合他社名や、使用を避けたい表現を指定します。</p>
                  </div>
                  ...
                  <div className="space-y-3 rounded-lg border bg-muted/20 p-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {settings.mustHaveWords.map((word) => (
                        <Badge key={word} variant="secondary" className="flex items-center gap-1 pr-1 border-primary/20 bg-primary/10 text-primary">
                          {word}
                          <button onClick={() => handleRemoveWord("mustHaveWords", word)} className="hover:bg-primary/20 rounded-full p-0.5">
                            <X className="w-3 h-3" />
                          </button>
                        </Badge>
                      ))}
                    ...
                    />
                    <p className="text-xs text-muted-foreground">必ず含めたいスローガンやキーワードを指定します。</p>
                  </div>
                </div>
                ...
                <div className="space-y-6 pt-4 border-t">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label>絵文字の使用</Label>
                      <p className="text-xs text-muted-foreground">生成される文章での絵文字の使用を許可するかどうか</p>
                    </div>
                    <Switch 
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">生成される文章でのハッシュタグの使用を許可するかどうか</p>
                    </div>
                    <Switch 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          onChange={(e) => handleMaxHashtagsChange("x", parseInt(e.target.value) || 0)}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          onChange={(e) => handleMaxHashtagsChange("instagram", parseInt(e.target.value) || 0)}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          onChange={(e) => handleMaxHashtagsChange("line", parseInt(e.target.value) || 0)}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <Select value={settings.contentLength} onValueChange={(v) => handleChange("contentLength", v)}>
          ...
          <TabsContent value="knowledge" className="space-y-6">
            <Card className="rounded-lg border-border/70 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>プロモーション等で頻繁に使用する固定情報を登録しておきます。</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <Textarea 
                    ...
                    className="min-h-[100px]"
                  />
                  <p className="text-xs text-muted-foreground">プロモーション投稿などの末尾に自動的に追加を検討する定型文です。</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
